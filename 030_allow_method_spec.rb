RSpec.describe 'allow method review' do
    it 'can customize return value for methods on doubles' do
        ...
        allow(calculator).to receive(:test).and_return(15)

        expect(calculator.test).to eq(15)
        expect(calculator.test(3)).to eq(15)
        expect(calculator.test(-2, 3, 5)).to eq(15)
        expect(calculator.test('hello')).to eq(15)
    ...
    it 'can return multiple return values in a sequence' do
